...
//                Integer stockNum= seckillGood.getStockCount()-num;
            ...
            seckillOrder.setStatus(SecKillOrderStatusConst.SECKILL_ORDER_STATUS_UNPAID + "");
            /*
...
*/
            ...
            //修改用户派对状态,秒杀成功,等待付款
            userRecord.setStatus(SecKillQueuingStatusConst.SECKILL_QUEUING_STATUS_WAITING_PAY);
            ...
            //发送延迟消息
            rabbitTemplate.convertAndSend("seckill_order_exchange", "seckill.order.dead", username, message -> {
                //设置有效期为5分钟
                MessageProperties messageProperties = message.getMessageProperties();
                messageProperties.setExpiration(5 * 60 * 1000 + "");
                return message;
            });
        } else {
            //修改用户的排队状态,写入失败的原因
...
//        updateUserRecord(userRecord);
    ...
    private void updateSecKillGoodsStockRedis(Integer num, SeckillGoods seckillGood, String time) {
        Long stockNum =
                redisTemplate.opsForHash().increment("seckill_goods_stock_count_" + time, seckillGood.getId() + "", -num);
        //设置剩余的库存
